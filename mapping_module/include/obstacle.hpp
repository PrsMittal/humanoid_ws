...
class object{
	public:	
		object(vect p, TYPE type): center(p), type(TYPE::type){}
		virtual vect potential(const vect& p, METHOD Method) = 0;
...
class wall : public object {
	public:
		...
		vect potential(const vect& p,METHOD Method); //elliptical potential field 
...
class well : public object {
	public: 
		...
		vect potential(const vect& p,METHOD Method); //circular potential field
		double distance(const vect& p){double d = (p-center).length(); return d;}
...
class door : public object {
	...
	vect potential(const vect& p,METHOD Method); //potential = d^2 * gaussian(d, sig), d= distance of point from center. Can something work better?? 
	double distance(const vect& p){double d = (p-center).length(); return d;}
...
class goal_line : public object {
	public: 
		...
		vect potential(const vect& p, METHOD Method);
