...
void shrink(const cv::Mat& src, cv::Mat& dest){
	...
	cv::Mat tempx(cv::Size(src.cols/2, src.rows), CV_32F, cv::Scalar(0));
	cv::Mat tdest(cv::Size(src.rows/2, src.cols/2), CV_8U, cv::Scalar(0));
	...
	for (int i=0; i<tempx.rows; i++){
		...
		for (int j=1; j<tempx.cols-1; j++){
			desti[j] += gx[0]*static_cast<float>(srci[2*j-2]); 
			desti[j] += gx[1]*static_cast<float>(srci[2*j-1]); 
			desti[j] += gx[2]*static_cast<float>(srci[2*j]);
			desti[j] += gx[3]*static_cast<float>(srci[2*j+1]);
			desti[j] += gx[4]*static_cast<float>(srci[2*j+2]);
		}
	}
